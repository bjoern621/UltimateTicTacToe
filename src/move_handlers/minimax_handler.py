...
class MinimaxHandler(MoveHandler):
    ...
    It can be depth-limited or time-limited (or both)."""
    ...
    def __init__(
        self, player: Player, max_depth: int = 5, max_time: float = 1.0
    ) -> None:
    ...
    def get_move(
        ...
        print(
            f"Minimax ({self.player}) chose move: {move} with score: {score}. Took {time.time() - start_time:.2f} seconds. Cache size: {-1}"
        )
